...
class ROICanvas(ft.Column):
    ...
    def __init__(
        ...
        self.canvas = cv.Canvas(
            shapes=[], width=self.display_width, height=self.display_height,
        )
        ...
        self.points_text = ft.Text("Puntos ROI: (click para agregar vertices)", size=12)

        self.btn_clear = ft.OutlinedButton(
            "Limpiar ROI", icon=ft.Icons.DELETE_OUTLINE, on_click=self._on_clear, disabled=True,
        )
        self.btn_close = ft.OutlinedButton(
            "Cerrar Poligono", icon=ft.Icons.CHECK_CIRCLE_OUTLINE,
            on_click=self._on_close_polygon, visible=False, disabled=True,
        )
        ...
        gesture = ft.GestureDetector(
            content=self.canvas, on_tap_down=self._on_tap_down, mouse_cursor=ft.MouseCursor.PRECISE,
        )
        ...
        self._image_container = ft.Container(
            content=ft.Stack(
                [
                    ft.Image(
                        src=self.image_base64, width=self.display_width,
                        height=self.display_height, fit=ft.BoxFit.FILL,
                    ),
                    gesture,
                ]
            ),
        ...
        self.controls = [
            ...
            self.points_text,
            ft.Row([self.btn_clear, self.btn_close, self.btn_confirm], spacing=10, wrap=True),
        ]
        self.spacing = 10
    ...
    def _update_buttons(self):
        ...
        can_close = (
            self.mode == ROI_MODE_POLYGON and len(self.points) >= 3 and not self.is_closed
        )
        ...
        self.points_text.value = f"{mode_label} ({len(self.points)} pts){status}: {coords_str}"
    ...
    def _redraw(self):
        ...
        if self.mode == ROI_MODE_POLYGON:
            if len(self.points) >= 3:
                path_elements = [cv.Path.MoveTo(self.points[0][0], self.points[0][1])]
                for px, py in self.points[1:]:
                    path_elements.append(cv.Path.LineTo(px, py))
                path_elements.append(cv.Path.Close())
                shapes.append(cv.Path(elements=path_elements, paint=self._fill_paint))
